...
- Model and effort IDs live in `prompts/models.json` (`reasoningEffort`; per-task models via `/config`; list IDs with `cmd --list-models`). Verify supported levels before scripting; a model without effort metadata falls back to its default. On a single-model setup such as DeepSeek V4.1 Flash (`low`, `high`, `max`), tiers differ by effort, not model.
...
- Verify discovery after deploy: the agent list shows the six roles and `/config` shows the expected per-task models.
