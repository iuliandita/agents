...
# A leading @ is an omp model-role alias such as @smol.
MODEL_RE = re.compile(r"^@?[A-Za-z0-9._:/+-]+$")
KEY_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_-]*$")
WRAPPER_RE = re.compile(r"^[A-Za-z0-9._/-]+$")

AGENT_HARNESSES = ("claude", "codex", "opencode", "commandcode", "antigravity", "omp")
...
SHADOWED_NAMES = {
    ...
    "omp": frozenset({"scout", "reviewer", "security-reviewer", "task", "sonic"}),
}

GENERATED_MARKER = "Generated by render-agents"
GUARD_NAME = "agents-shell-ro-guard.py"
...
def validate_overrides(data: object, source: Path) -> dict:
    ...
    for harness, entry in data.items():
        ...
        for wrapper in wrappers:
            if not isinstance(wrapper, str) or not WRAPPER_RE.match(wrapper):
                raise SystemExit(
                    f"{label}: '{harness}.shell_ro_wrappers' entry {wrapper!r} must match {WRAPPER_RE.pattern}"
                )
...
def check_override_agents(overrides: dict, specs: list[AgentSpec]) -> None:
    ...
    for harness, entry in overrides.items():
        for name in entry.get("agents", {}):
            if name not in known:
                raise SystemExit(
                    f"prompts/models.local.json: '{harness}.agents.{name}' does not match any agent in agents/"
                )
...
# omp's read tool also fetches URLs, so web maps to web_search plus read.
OMP_TOOLS = {
    "read": ("read",),
    "search": ("grep", "glob"),
    "edit": ("edit",),
...
def generated_header(spec: AgentSpec, invariants: str, salt: str = "") -> str:
    digest = hashlib.sha256((invariants + "\n" + spec.source_text + "\n" + salt).encode("utf-8")).hexdigest()[:12]
    return (
...
def markdown_body(spec: AgentSpec, invariants: str, salt: str = "") -> str:
...
def render_codex(spec: AgentSpec, resolved: Resolved, invariants: str) -> str:
    ...
    salt = f"codex:{resolved.model}:{resolved.effort}"
    lines = [
        f"# {generated_header(spec, invariants, salt)}",
...
def render_opencode(spec: AgentSpec, resolved: Resolved, invariants: str) -> str:
    ...
    lines.append("---")
    salt = f"opencode:{resolved.model}:{resolved.effort}"
    return "\n".join(lines) + "\n" + markdown_body(spec, invariants, salt=salt)
...
def render_omp(spec: AgentSpec, resolved: Resolved, invariants: str) -> str:
    # A flow list keeps an empty expansion explicit: omp reads a bare `tools:` as
    # unrestricted but `tools: []` as no tools.
    tools = expand_tools(spec.tools, OMP_TOOLS)
    lines = [
        "---",
        f"name: {spec.name}",
        f"description: {yaml_string(spec.description)}",
    ...
    if resolved.effort_key:
        lines.append(f"{resolved.effort_key}: {resolved.effort}")
    lines.append("---")
    salt = f"omp:{resolved.model}:{resolved.effort}"
    return "\n".join(lines) + "\n" + markdown_body(spec, invariants, salt=salt)
...
TARGET_DIRS = {
    ...
    # omp resolves task agents from HOME (or a profile), not PI_CODING_AGENT_DIR.
