...
def render_document(
    ...
    rendered = (
        ...
        "Edit prompts/core.md and prompts/harnesses/*.md, plus optional prompts/local.md and "
        "prompts/private.md (see the .example.md templates), not rendered files. -->\n\n"
        f"{fragment.rstrip()}\n\n"
        f"{core.rstrip()}\n"
    )
    for overlay in (local, private):
        if overlay.strip():
            rendered += f"\n{overlay.rstrip()}\n"
    return rendered
...
def read_overlay(repo_root: Path, name: str) -> str:
...
PROMPT_MARKER = "Generated by sync-ai-prompts"
...
# paths, and the model provider behind a harness is the user's choice, so it reaches only the
# harnesses listed in AGENTS_PRIVATE_HARNESSES or prompts/private-harnesses.txt ("all" for
# every one). There is no built-in default.
ALL_HARNESSES_KEYWORD = "all"
...
def private_harnesses(repo_root: Path, env: dict[str, str] | None = None) -> frozenset[str]:
    ...
    if override.strip():
        names = [part.strip() for part in override.replace(",", "\n").split() if part.strip()]
    else:
        path = repo_root / "prompts" / PRIVATE_HARNESSES_FILE
        if not path.exists():
            return frozenset()
        names = [
            line.strip()
...
def global_target(harness: Harness) -> bool:
    # Manual targets write project-level files that may be committed, so they never carry overlays.
    return harness.support_level == DEPLOYABLE


def withheld_notice(overlays: Overlays, names: list[str]) -> str | None:
    if not overlays.private.strip():
        return None
    withheld = [name for name in names if name not in overlays.trusted]
    if not withheld:
        return None
    return (
        f"notice: prompts/private.md withheld from {', '.join(withheld)}; list trusted harnesses in "
        f"prompts/{PRIVATE_HARNESSES_FILE} or {PRIVATE_HARNESSES_ENV} (use '{ALL_HARNESSES_KEYWORD}' for every one)"
    )
...
def render_for(
    ...
    overlays: Overlays,
    stamp: str | None = None,
    with_overlays: bool | None = None,
) -> str:
    if with_overlays is None:
        with_overlays = global_target(harness)
    return render_document(
        read_fragment(repo_root, harness),
        core,
        private=overlays.private if with_overlays and harness.name in overlays.trusted else "",
        stamp=stamp,
        enabled_optional=frozenset(harness.optional_blocks),
        local=overlays.local if with_overlays else "",
    )
...
def enforce_budget(harness: Harness, rendered: str) -> None:
    ...
    if size > harness.max_bytes:
        raise SystemExit(
            ...
            "trim the fragment or the local/private overlays"
        )
...
def render_all(
    ...
    notice = withheld_notice(overlays, [harness.name for harness in harnesses if global_target(harness)])
    if notice:
        print(notice)
    ...
    for harness in harnesses:
        ...
        rendered = render_for(repo_root, harness, core, overlays, stamp)
...
def deploy(
    ...
    notice = withheld_notice(overlays, [harness.name for harness in resolved if global_target(harness)])
    if notice:
        print(notice)
